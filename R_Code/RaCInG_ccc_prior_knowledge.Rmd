...
| -----------------------  | ----------------------------------- | ----------------------------------- |
| Liana | Inter-cellular interactions | Türei et al. Molecular systems biology, 2021; Dimitrov et al. Nat Commun, 2022 |
| Ramilowski | RNA-seq data from 144 cell types | Ramilowski et al., Nat Comm, 2015 |
| Deconvolution computational methods | Quantification of cell type fractions | .... |
...
> Retrieve LIANA's consensus resource of inter-cellullar communication network (`liana` R package version 0.1.8)

- LIANA's Consensus Network includes expert-curated (in the context of CCC) interactions from “CellPhoneDB”, “CellChatDB”, “ICELLNET”, “connectomeDB2020”, and “CellTalkDB” resources.
...
# curated_omni <- select_resource("Consensus")[[1]]
...
- Conclusion:
`get_curated_omni()` returns, via OmniPath, the most up-to-date version of all databases.
`select_resource()` retrieves the network stored by the developers.
...
# Cell type fractions obtained for TCGA GBM dataset
celltype_quantification <- read.csv(file = "../input_data_model1/GBM_TMEmod_cell_fractions.csv",
                                    row.names = 1)
...
LRpairs <- data.frame(LRpairs=unique(paste0(tmp$Ligand, "_", tmp$Receptor)))

# export LRpairs to excel file
write.csv(LRpairs, file = "../input_data_model1/LRpairs.csv")
